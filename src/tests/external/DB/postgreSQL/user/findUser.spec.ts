...
describe("find user in database", () => {       
    ...
    it("should be successfully finding the user", async() => {
        ...
        const sut = await new findUser(configDB).find(userDB.body.data.user.id_user)
    ...
    it("should be error because the user was not found", async() => {
        ...
        const sut = await new findUser(configDB).find("6d9b4e2b-b496-4082-9023-176f618cb244")
    ...
    it("should be error because the value passed is not an ID", async() => {
        ...
        const sut = await new findUser(configDB).find("id_criado_exclusivamente_para_este_teste")
